...
module.exports = {
    ...
    async execute(bot) {
        ...
        const messages3 = await bot.channels.cache.get(require('../../config.json').channel.join2).messages.fetch({ limit: 99 });

        if(messages.size !== 0) await bot.channels.cache.get(require('../../config.json').channel.voc2).bulkDelete(messages, true);
        if(messages2.size !== 0) await bot.channels.cache.get(require('../../config.json').channel.msg2).bulkDelete(messages2, true);
        if(messages3.size !== 0) await bot.channels.cache.get(require('../../config.json').channel.join2).bulkDelete(messages3, true);

        const msg = await bot.channels.cache.get(require('../../config.json').channel.voc2).send({ content: `**Veuillez patienter... ⏳**` })
        const msg2 = await bot.channels.cache.get(require('../../config.json').channel.msg2).send({ content: `**Veuillez patienter... ⏳**` })
        const msg3 = await bot.channels.cache.get(require('../../config.json').channel.join2).send({ content: `**Veuillez patienter... ⏳**` })
        setInterval(async () => {
            let leaderboard = [];
